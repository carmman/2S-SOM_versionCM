function [sMap,bmus,H] = som_make_Fusion(D, varargin)
...
while i<=length(varargin), 
  ...
  if ischar(varargin{i}), 
    switch varargin{i}, 
     ...
     case 'radius_ini', i=i+1; radius_ini=varargin{i};
     case 'radius_fin', i=i+1; radius_fin=varargin{i};
      % unambiguous values
...
switch func,
 ...
 case 'batch',  [sMap,bmus,H] = som_batchtrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
end
...
switch func,
 case 'seq',    sMap = som_seqtrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
 case 'sompak', sMap = som_sompaktrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
 case 'batch',  [sMap,bmus,H] = som_batchtrain(sMap,D,sTrain,'tracking',tracking,'mask',mask,'trainlen',50);
end
