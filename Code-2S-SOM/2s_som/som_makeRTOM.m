function [sMap bmus Alpha Beta] = som_makeRTOM(D,varargin)
...
switch func,
 ...
 case 'batch',  [sMap bmus Alpha Beta] = som_batchtrainRTOM(sMap,D,'TypeAlgo',TypeAlgo,UpdateRule,[1 1],1,DimCategorieVar,'DimData',DimData,'DimBloc',DimBloc,'lambda',lambda,'eta',eta,'trainlen',15, 'radius_ini',3,'radius_fin',0.5); 
     %[sMap bmus] =            som_batchtrainRTOM(sMap,D,UpdateRule,
     %nb_var, nb_mod, sTrain,'tracking',tracking,'mask',mask,'TypeAlgo',TypeAlgo,'DimData',DimData,'SousDimData',SousDimData);
end
...
switch func,
 case 'seq',    sMap = som_seqtrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
 case 'sompak', sMap = som_sompaktrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
 case 'batch',  [sMap bmus Alpha Beta] = som_batchtrainRTOM(sMap,D,'TypeAlgo',TypeAlgo,UpdateRule,[1 1],1,DimCategorieVar,'DimData',DimData,'DimBloc',DimBloc,'lambda',lambda,'eta',eta,'trainlen',25); 
     %[sMap bmus] = som_batchtrainRTOM(sMap, D,UpdateRule,nb_var,nb_mod,sTrain,'tracking',tracking,'mask',mask,'TypeAlgo',TypeAlgo,'DimData',DimData,'SousDimData',SousDimData);
end
