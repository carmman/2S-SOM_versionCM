function sMap = som_map_structRT(dim, varargin)

%SOM_MAP_STRUCT Create map struct whith categorical variables. 
% 
...
% size_vble est un vecteur qui contient le nombre des vbles cat et quant
%par défaut tout les vbles sont quant
size_vble=[dim,0]
% varargin
i=1; 
while i<=length(varargin), 
  ...
  if ischar(varargin{i}), 
    switch varargin{i}, 
     ...
     %%%%%%%%RT for categorical variables%%%%%%%%%%%%%
     case 'mixed',i=i+2;size_vble=varargin{i-1};nb_modality=varargin{i};
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     otherwise argok=0; 
    end
...
if(ne(sum(size_vble),dim))
    fprintf('error sum of variables different of dim')
    size_vble=[dim 0];
end
if((exist('nb_modality','var'))&&(ne(size_vble(2),0)))
    fprintf('exiiiiissst');
    dim2=size_vble(1)+sum(nb_modality);
    fprintf('dimmmmmm2\n%d',dim2)
comp_names = cell(dim2,1); 
for i = 1:dim2, comp_names{i} = sprintf('Variable%d', i); end
comp_norm  = cell(dim2,1);
else
...
% create the SOM
%size_vble(1) présente les variables quantitatives, générer ces variables
%avec rand
codebook1 = rand(prod(sTopol.msize),size_vble(1)); 
%pour les variables catégorielles
if(ne(size_vble(2),0))
    codebook2=[];
    nb_ref=prod(sTopol.msize);
    for i=1:size_vble(2)
        nb_mod=nb_modality(i);
   code=zeros(nb_ref,nb_mod);
   for j=1:nb_ref
   VECT=randperm(nb_mod);
   code(j,VECT(1))=1/sqrt(2);
   end
   codebook2=[codebook2 code];
    
    end
end
if(exist('codebook2','var'))
    fprintf('exist codebook2')
    codebook2
