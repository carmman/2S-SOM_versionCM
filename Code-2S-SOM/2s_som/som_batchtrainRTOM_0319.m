	 ...
	 % pr? traitement : creation des diff?retentes blocs de variables  
         ...
         for i=1:size(DimData,2) % iteration sur les  blocs de variables
            ...
            % disp(deb); disp(fin );
             ...
             % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             %BlocData(i).Dim=DimBloc(i).Dim;  %  %%%% BUG ????
             BlocData(i).Dim=DimBloc(i).Dim;  %  %%%% CORRECTION ????
             % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                for i=1:size(DimData,2)
                    ...
                    switch UpdateRule,
                            case 'numeric', [ BlocData(i).M ] = UpdateNumericOM( H,P,BlocData(i).D,BlocData(i).Known, BlocData(i).M);
                                ...
                                % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                PoidsAlpha(i).SB(1).alpha=repmat(ones(1,BlocData(i).Dim(1))./size(DimData,2),blen,1); %  %%%% BUG ????
                                %PoidsAlpha(i).SB(1).alpha=(1/size(DimData,2))*ones(blen,size(DimData,2));  %  %%%% CORRECTION ????
                                % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                disp('numeric')
                            case 'mixed1',
                                %disp(i)
                                ...
                                % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                PoidsAlpha(i).SB(1).alpha=repmat(ones(1,BlocData(i).Dim(1))./size(DimData,2),blen,1); %  %%%% BUG ????
                                PoidsAlpha(i).SB(2).alpha=repmat(ones(1,BlocData(i).Dim(2))./size(DimData,2),blen,1); %  %%%% BUG ????
                                %PoidsAlpha(i).SB(1).alpha=(1/size(DimData,2))*ones(blen,size(DimData,2));  %  %%%% CORRECTION ????
                                %PoidsAlpha(i).SB(2).alpha=(1/size(DimData,2))*ones(blen,size(DimData,2));  %  %%%% CORRECTION ????
                                % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                DebCat=1;
