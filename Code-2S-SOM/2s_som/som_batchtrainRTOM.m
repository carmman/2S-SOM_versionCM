  ...
  switch TypeAlgo,
      ...
      for t = 1:trainlen,
        ...
        switch UpdateRule,
          ...
          case 'mixed1',
            ...
            F(nonzero) = Squal(nonzero)./ Aqual(nonzero); %% les fr?quences de chaque modalit?
    ...
    case '2SSOM',
      ...
      for i=1:size(DimData,2) % iteration sur les  blocs de variables
        ...
        debBloc=1;
        for k=1:length(DimBloc(i).Dim), % Cr?ation des blocs de variables
          ...
          BlocData(i).SousBlocData(k).W1 = (BlocData(i).SousBlocData(k).mask * ones(1,dlen)) ...
              .* BlocData(i).SousBlocData(k).Known'; % ok
          BlocData(i).SousBlocData(k).D(~BlocData(i).SousBlocData(k).Known) = 0; % ok
          BlocData(i).SousBlocData(k).WD = 2 * diag(BlocData(i).SousBlocData(k).mask) ...
              * BlocData(i).SousBlocData(k).D'; % ok
          BlocData(i).SousBlocData(k).dconst = (((BlocData(i).SousBlocData(k).D) .^ 2) ...
                                                * BlocData(i).SousBlocData(k).mask)'; % ok
      ...
      for t = 1:trainlen,
        ...
        if t==1,
          %% Affectation des observations aux cellules
          %% verification OK
          % calcul des distances entre les observations et vecteurs r?f?rents r
          i0 = 0;
          while i0+1<=dlen,
            inds = (i0+1):min(dlen,i0+blen); i0 = i0+blen;
            ...
            for i=1:size(DimData,2) % distance
              for k=1:length(DimBloc(i).Dim),
                if k==1
                  Dist = Dist + ( (BlocData(i).SousBlocData(k).M .^ 2) * ...
                                  BlocData(i).SousBlocData(k).W1(:,inds) ...
                                  - BlocData(i).SousBlocData(k).M *  ...
                                  BlocData(i).SousBlocData(k).WD(:,inds) ...
                                  + BlocData(i).SousBlocData(k).W2 * ...
                                  BlocData(i).SousBlocData(k).D2(:,inds)) ...
                         / DimBloc(i).Dim(k);
                else
                  Dist = Dist + ( (BlocData(i).SousBlocData(k).M .^ 2) * ...
                                  BlocData(i).SousBlocData(k).W1(:,inds) ...
                                  ...
                                  + BlocData(i).SousBlocData(k).W2 * ...
                                  BlocData(i).SousBlocData(k).D2(:,inds)) ...
                         * DimBloc(i).Dim(k) / DimBloc(i).Dim(k);
                  % Expression simplifiee :
                  % Dist = Dist+ ((M .^ 2) * W1(:,inds) - ...
                  %               M * WD(:,inds) + ...
                  %               W2 * D2(:, inds)) * ...
                  %        Dim(k) / Dim(k);   <--- etrange, ces deux termes s'eliminent
                end
              end
            end
            %Dist=(Dist'*H)';
          ...
          for i=1:size(DimData,2)
            ...
            switch UpdateRule,
              case 'numeric',
                [ BlocData(i).M ] = UpdateNumericOM( ...
                    H,P,BlocData(i).D,BlocData(i).Known, BlocData(i).M);
                PoidsBeta(i).SB(1).beta = repmat(ones(1,BlocData(i).Dim(1)) ...
                                                 ./ DimData(i), blen, 1);
                PoidsAlpha(i).SB(1).alpha = repmat(ones(1,BlocData(i).Dim(1)) ...
                                                   ./ size(DimData,2), blen, 1);
                %disp('numeric')
              case 'mixed1',
                %disp(i)
                PoidsBeta(i).SB(1).beta = repmat(ones(1,BlocData(i).Dim(1)) ...
                                                 ./ DimData(i), blen, 1);
                PoidsBeta(i).SB(2).beta = repmat(ones(1,BlocData(i).Dim(2)) ...
                                                 ./ DimData(i), blen, 1);
                PoidsAlpha(i).SB(1).alpha = repmat(ones(1,BlocData(i).Dim(1)) ...
                                                   ./ size(DimData,2), blen, 1);
                PoidsAlpha(i).SB(2).alpha = repmat(ones(1,BlocData(i).Dim(2)) ...
                                                   ./ size(DimData,2), blen, 1);
                %DebCat=1;
                % Normalisation par rapport aux modalit?s
                % if DimCategorieVar(i).Dim==0
                % else
                %   for Cat=1:length(DimCategorieVar(i).Dim)
                %     FinCat=sum(DimCategorieVar(i).Dim(1:Cat));
                %     PoidsBeta(i).SB(2).beta(:,DebCat:FinCat)=(1/DimCategorieVar(i).Dim(Cat))* ...
                %        PoidsBeta(i).SB(2).beta(:,DebCat:FinCat);
                %     DebCat=FinCat+1;
                %   end
                % end
                [ BlocData(i).SousBlocData(1).M ] = UpdateNumericOM( ...
                    H,P, ...
                    BlocData(i).SousBlocData(1).D,BlocData(i).SousBlocData(1).Known, ...
                    BlocData(i).SousBlocData(1).M);  % quantitative Part
                ...
                %BlocData(i).SousBlocData(1).M=reshape(BlocData(i).SousBlocData(1).M,size(H,1), ...
                %                                      size(BlocData(i).SousBlocData(1).D,2));
                %BlocData(i).SousBlocData(2).M=reshape(BlocData(i).SousBlocData(2).M,size(H,1), ...
                %                                      size(BlocData(i).SousBlocData(2).D,2));
        ...
        if t>1,
          ...
          while i0+1<=dlen,
            ...
            for i=1:size(DimData,2) % distance
              if strcmp(UpdateRule,'mixed1')
                for k=1:2,
                  if k==1
                    Dist = Dist + ( ( (PoidsBeta(i).SB(k).beta .* PoidsAlpha(i).SB(k).alpha) .* ...
                                      (BlocData(i).SousBlocData(k).M) .^ 2) * ...
                                    BlocData(i).SousBlocData(k).W1(:,inds) ...
                                    - (PoidsBeta(i).SB(k).beta .* PoidsAlpha(i).SB(k).alpha) .*  ...
                                    BlocData(i).SousBlocData(k).M  * ...
                                    BlocData(i).SousBlocData(k).WD(:,inds) ...
                                    + (PoidsBeta(i).SB(k).beta .* PoidsAlpha(i).SB(k).alpha) .*  ...
                                    BlocData(i).SousBlocData(k).W2 * ...
          ...
          for i=1:size(DimData,2)
            ...
            switch UpdateRule,
              case 'numeric',
                [ BlocData(i).M ] = UpdateNumericOM( ...
                    H ,P, BlocData(i).D,BlocData(i).Known,BlocData(i).M); %ok
              case 'mixed1',
                [ BlocData(i).SousBlocData(1).M ] = UpdateNumericOM( ...
                    H, P, ...
                    BlocData(i).SousBlocData(1).D,BlocData(i).SousBlocData(1).Known, ...
          ...
          Psi_ck = zeros(blen,size(DimData,2));
          Beta = struct;
          Beta_ck = struct;
          for i=1:size(DimData,2) % distance
                                  % Les alpha
            ...
            if strcmp(UpdateRule,'mixed1')
              for k=1:2,
                DistAlpha = DistAlpha ...
                    + (PoidsBeta(i).SB(k).beta .* (BlocData(i).SousBlocData(k).M) .^ 2) * ...
                    BlocData(i).SousBlocData(k).W1 ...
                    -  PoidsBeta(i).SB(k).beta .*  BlocData(i).SousBlocData(k).M  * ...
                    BlocData(i).SousBlocData(k).WD ...
                    +  PoidsBeta(i).SB(k).beta .*  BlocData(i).SousBlocData(k).W2 * ...
                    BlocData(i).SousBlocData(k).D2 ...
                    / DimBloc(i).Dim(k);
              end
            end
            if strcmp(UpdateRule,'numeric')
            ...
            for c=1:blen,
              if strcmp(UpdateRule,'mixed1')
                for k=1:2
                  if(DimCategorieVar(i).Dim==0)
                    Phi = (PoidsAlpha(i).SB(1).alpha(c,i) * ((repmat(H(c,bmus),DimData(i),1))') .* ...
                           ( [BlocData(i).SousBlocData(1).D] ...
                             - repmat([BlocData(i).SousBlocData(1).M(c,:)],dlen,1) ) .^ 2) ...
                          / DimBloc(i).Dim(1);
                  else
                    Phi = (PoidsAlpha(i).SB(k).alpha(c,i) * ((repmat(H(c,bmus),DimData(i),1))') .* ...
                           ( [BlocData(i).SousBlocData(1).D BlocData(i).SousBlocData(2).D] - ...
                             repmat([BlocData(i).SousBlocData(1).M(c,:) ...
                                     BlocData(i).SousBlocData(2).M(c,:)],dlen,1) ) .^ 2) ...
                          / DimBloc(i).Dim(k);
                  end
                end
              ...
              if strcmp(UpdateRule,'numeric')
                Phi = PoidsAlpha(i).SB(k).alpha(c,1) * ((repmat(H(c,bmus),DimData(i),1))') ...
                      .* ( [BlocData(i).SousBlocData(1).D] ...
                           - repmat([BlocData(i).SousBlocData(1).M(c,:)],dlen,1)) .^ 2;
                Beta_ck(i).Beta_ck(c,:) = exp(-sum(Phi) / eta) / sum(exp(-sum(Phi) / eta));
              end
          ...
          Alpha_ck = exp(-Psi_ck / lambda) ./ repmat(sum((exp(-Psi_ck / lambda))'), ...
                                                     size(DimData,2),1)';
          deb=1;
          for i=1:size(DimData,2)
            fin=sum(DimData(1:i));
