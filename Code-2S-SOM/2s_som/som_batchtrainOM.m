function [sMap, bmus, sTrain] = som_batchtrainOM(sMap, D, varargin)

%SOM_BATCHTRAIN  Use batch algorithm to train the Self-Organizing Map.
%          OM=som_batchtrainOM(sMap, D,'mixed1',[5 2],[4 2])
    ...
    switch varargin{i}, 
     ...
     case 'mixed1',i=i+2; vect1=varargin{i-1};
         if(ne(length(vect1),2))
            ...
            UpdateRule='mixed1';
            case 'mixed2',i=i+3;vect1=varargin{i-2};
...
for t = 1:trainlen,  
  ...
  switch UpdateRule,
      ...
      case 'mixed1',
          % Separation of  quantitative and qualitative parts of the M
          Dquant=D(:,1:vect1(1));
          Dqual=D(:,(vect1(1)+1):end);
          Mquant=M(:,1:vect1(1));
          Mqual=M(:,(vect1(1)+1):end);
          F=M(:,(vect1(1)+1):end);
           Squant = H*(P*Dquant); 
          Aquant = H*(P*Known(:,1:vect1(1)));
          ...
          [ Mqual] = Mqualitative( F,H,Mqual,vect2 );
          Mquant=reshape(Mquant,size(H,1),size(Dquant,2));
          Mqual=reshape(Mqual,size(H,1),size(Dqual,2));
          M=[Mquant Mqual];
      case 'mixed2',
          % Separation of  quantitative qualitative and ordinal parts of M
          Dquant=D(:,1:vect1(1));
          Dqual=D(:,(vect1(1)+1):vect1(1)+vect1(2));
          Dordi=D(:,(vect1(1)+vect1(2)+1):end);
          Mquant=M(:,1:vect1(1));
          Mqual=M(:,(vect1(1)+1):vect1(1)+vect1(2));
          Mordi=M(:,(vect1(1)+vect1(2)+1):end);
          F=Mqual;
          F2=Mordi;
          Squant = H*(P*Dquant); 
          Aquant = H*(P*Known(:,1:vect1(1)));
          ...
          % Update qualitative part of M
          Squal = H*(P*Dqual); 
          Aqual = H*(P*Known(:,vect1(1)+1:vect1(1)+vect1(2)));
          nonzero = find(Aqual > 0); 
          F(nonzero) = Squal(nonzero)./ Aqual(nonzero);
          [ Mqual] = Mqualitaive( F,H,Mqual,Vect2 );
          Mquant=reshape(Mquant,size(H,1),size(Dquant,2));
          Mqual=reshape(Mqual,size(H,1),size(Dqual,2));
          
          Squal = H*(P*Dordi); 
          Aqual = H*(P*Known(:,vect1(1)+vect1(2):end));
          nonzero = find(Aqual > 0); 
          ...
          %mat_modal=..........
          [ Mordi] = Mordinal( F2,H,Vect3,mat_modal );
...
%   
%   %% Update qualitative part of M
%   % Determination of the frequence of each madality in the voronoi space of
%   % the neurone
%   % Fmod=m = sum_i (h_i * indicator(xik=modaliték)) / sum_i (h_i)
%   
%   Squal = H*(P*Dqual); 
%   Aqual = H*(P*Known(:,vect1(1)+1:end));
%   nonzero = find(Aqual > 0); 
...
%   % essaie de comprendre cette partie du code 
%   Mqual=F*0;
%   Vect2=[4 2];
%   deb=1; compt=1;
