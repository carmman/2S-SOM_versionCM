...
addpath(['.' filesep '2s_somtoolboxmixed'])
addpath(['.' filesep '2s_somtoolboxmixed' filesep 'ConsensensClustering'])
addpath(['.' filesep '2s_somtoolboxmixed' filesep 'NCEstimation'])
...
[sMap1 Bmus]=som_make(D,'init','randinit');  %% Initialisation des param?tres de SOM
...
for i=1:M
    for j=1:M
        for k=1:K
            [sMap1 Bmus]=som_make(D,'init','randinit');  %% Initialisation des param?tres de SOM
            [Result(i,j).sMap(k).sMap Result(i,j).bmus(k).bmus Result(i,j).Alpha(k).Alpha Result(i,j).Beta(k).Beta]=SS_SOM(sMap1, D,'TypeAlgo','2SSOM','DimData',DimData,'DimBloc',DimBloc,'lambda',2,'eta',50,'trainlen',50);
        end
    end
end 
%% Recup?ration de classes suite ? une CAH sur la sortie 2S-SOM
...
% Recup?ration des index particuliers 
...
% Apprentissage Kmeansi

for k=1:20
    ClKmeans(:,k)=kmeans(D,Nbclust);
...
for k=1:20
    [SOM(k).sMap1 SOM(k).Bmus]=som_make(D,'init','randinit');  %% Initialisation des param?tres de SOM
...
% Structuraton de la base les noise en derni?res position 
...
for l=1:M
    if l==1
        Index(:,l)=sort(randsample(2126, 2126, false));
        Z_l(l).D=D( Index(:,l),:);
        [sMap1 Bmus]=som_make(Z_l(l).D,'init','lininit');  %% Initialisation des param?tres de SOM
        [Z_l(l).sMap Z_l(l).bmus Z_l(l).Alpha Z_l(l).Beta ]=SS_SOM(sMap1, Z_l(l).D,'TypeAlgo','2SSOM','DimData',DimData,'DimBloc',DimBloc,'lambda',1*10+1,'eta',9*10+1,'trainlen',50);
    else
    Index(:,l)=sort(randsample(2126, 2126, true));
    Z_l(l).D=D( Index(:,l),:);
    [sMap1 Bmus]=som_make(Z_l(l).D,'init','lininit');  %% Initialisation des param?tres de SOM
    [Z_l(l).sMap Z_l(l).bmus Z_l(l).Alpha Z_l(l).Beta ]=SS_SOM(sMap1, Z_l(l).D,'TypeAlgo','2SSOM','DimData',DimData,'DimBloc',DimBloc,'lambda',1*10+1,'eta',9*10+1,'trainlen',50);
    end
...
for i=2:10
    for j=1:10
        for k=1:10
            [sMap1 Bmus]=som_make(Dreduit,'init','randinit');  %% Initialisation des param?tres de SOM
