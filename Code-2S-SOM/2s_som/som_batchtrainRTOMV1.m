function [sMap, bmus, Alpha_ck Beta] = som_batchtrainRTOMV1(sMap,TypeAlgo,UpdateRule, D,DimCategorieVar,DimData,DimBloc,lambda,eta,trainlen)
                     ...
                     for i=1:size(DimData,2) % distance 
                         for k=1:length(DimBloc(i).Dim),
                             if k==1
                               ...
                               Dist =Dist+(((BlocData(i).SousBlocData(k).M).^2)* BlocData(i).SousBlocData(k).W1(:,inds) -  BlocData(i).SousBlocData(k).M* BlocData(i).SousBlocData(k).WD(:,inds)+ BlocData(i).SousBlocData(k).W2*(BlocData(i).SousBlocData(k).D2(:,inds)))*DimBloc(i).Dim(k)/DimData(i);  
                             end 
                         end 
                     end 
                     %Dist=(Dist'*H)';
...
%                                 end
                                ...
                                %F=reshape(F,size(H,1),sum(DimBloc(i).Dim));
             ...
             if t>1,
                 ...
                 while i0+1<=dlen, 
                     ...
                     for i=1:size(DimData,2) % distance 
                         if strcmp(UpdateRule,'mixed1')
                             for k=1:2,
                                 if k==1
                                     Dist =Dist+((((PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha)).*(BlocData(i).SousBlocData(k).M).^2)* BlocData(i).SousBlocData(k).W1(:,inds) -  (PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha).*BlocData(i).SousBlocData(k).M* BlocData(i).SousBlocData(k).WD(:,inds)+ (PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha).*BlocData(i).SousBlocData(k).W2*(BlocData(i).SousBlocData(k).D2(:,inds)));
                                 else
                                     Dist =Dist+((((PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha)).*(BlocData(i).SousBlocData(k).M).^2)* BlocData(i).SousBlocData(k).W1(:,inds) -  (PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha).*BlocData(i).SousBlocData(k).M* BlocData(i).SousBlocData(k).WD(:,inds)+ (PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha).*BlocData(i).SousBlocData(k).W2*(BlocData(i).SousBlocData(k).D2(:,inds)))*DimBloc(i).Dim(k)/DimData(i);  
                                 end 
                              end  
                         end
                ...
                for i=1:size(DimData,2)
                    ...
                    switch UpdateRule,
                            ...
                            case 'mixed1',
                                ...
                                nonzero = find(Aqual > 0); 
                                F(nonzero) = Squal(nonzero)./Aqual(nonzero); %% les fréquences de chaque modalité
                               % F=reshape(F,size(H,1),size(BlocData(i).SousBlocData(2).D,2));
                                [BlocData(i).SousBlocData(2).M] = MqualitativeRTOM( F, H, DimCategorieVar(i).Dim);
                                BlocData(i).SousBlocData(1).M=reshape(BlocData(i).SousBlocData(1).M,size(H,1),size(BlocData(i).SousBlocData(1).D,2));
                                BlocData(i).SousBlocData(2).M=reshape(BlocData(i).SousBlocData(2).M,size(H,1),size(BlocData(i).SousBlocData(2).D,2));
                                BlocData(i).M=[BlocData(i).SousBlocData(1).M BlocData(i).SousBlocData(2).M];
                ...
                for i=1:size(DimData,2) % distance 
                         ...
                         if strcmp(UpdateRule,'mixed1')
                             for k=1:2,
                                 DistAlpha = DistAlpha+((PoidsBeta(i).SB(k).beta).*(BlocData(i).SousBlocData(k).M).^2)* BlocData(i).SousBlocData(k).W1 -  (PoidsBeta(i).SB(k).beta).*BlocData(i).SousBlocData(k).M* BlocData(i).SousBlocData(k).WD+ (PoidsBeta(i).SB(k).beta).*BlocData(i).SousBlocData(k).W2*(BlocData(i).SousBlocData(k).D2); 
                             end 
                         end
                         if strcmp(UpdateRule,'numeric')
                                 k=1;
                         ...
                         for c=1:blen,
                            if strcmp(UpdateRule,'mixed1')
                                if(DimCategorieVar(i).Dim==0) 
                                    Phi=PoidsAlpha(i).SB(1).alpha(c,i)*((repmat(H(c,bmus),DimData(i),1))').*([BlocData(i).SousBlocData(1).D]-repmat([BlocData(i).SousBlocData(1).M(c,:)],dlen,1)).^2; 
                                else
                                    Phi=PoidsAlpha(i).SB(k).alpha(c,i)*((repmat(H(c,bmus),DimData(i),1))').*([BlocData(i).SousBlocData(1).D BlocData(i).SousBlocData(2).D]-repmat([BlocData(i).SousBlocData(1).M(c,:) BlocData(i).SousBlocData(2).M(c,:)],dlen,1)).^2; 
                                end
