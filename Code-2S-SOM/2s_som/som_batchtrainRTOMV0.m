function [sMap, bmus, sTrain] = som_batchtrainRTOM(sMap, D, varargin)
...
while i<=length(varargin), 
  ...
  if ischar(varargin{i}), 
    switch varargin{i}, 
     % argument IDs
        case 'mixed1',i=i+2; vect1=varargin{i-1};
     ...
     case 'NCSOM',
        ...
        switch UpdateRule,
            ...
            case 'mixed2',
                % Update qualitative part of M
                Dquant=D(:,1:vect1(1));
                [ Mquant ] = UpdateNumericOM( H,P,Dquant,Known(:,1:vect1(1))); % OK
                % Update qualitative part of M
                Dqual=D(:,(vect1(1)+1):vect1(1)+sum(vect2));
                Squal = H*(P*Dqual); 
                Aqual = H*(P*Known(:,vect1(1)+1:(vect1(1)+sum(vect2))));
                nonzero = find(Aqual > 0); 
                F(nonzero) = Squal(nonzero)./ Aqual(nonzero);
                [Mqual] = MqualitativeRTOM( F, H, vect2 );      
                % Update ordinal part of M
                % mat_modal est une matrice pour chaque variable ordinale elle
                %donne les différentes modalité,
                %Ici il faut choisir de définir cette matrice ou de l'ajouter
                %comme paramètre de la fonction som_batchtrain
                %mat_modal=.....
                Dordi=D(:,(vect1(1)+ sum(vect2)+1):end);
                Squal = H*(P*Dordi); 
                Aqual = H*(P*Known(:,(vect1(1)+sum(vect2)+1):end));
                nonzero = find(Aqual > 0); 
                F2(nonzero) = Squal(nonzero)./ Aqual(nonzero);
                F2=reshape(F2,size(H,1),size(Dordi,2));
                [ Mordi] = Mordinal( F2,vect3,mat_modal);
                Mquant=reshape(Mquant,size(H,1),size(Dquant,2));
                Mqual=reshape(Mqual,size(H,1),size(Dqual,2));
                M=[Mquant Mqual Mordi];
        end
        end
     case '2SSOM',
         deb=1; BlocData=struct;
         for i=1:size(DimData,2)
             fin=sum(DimData(1:i));
             BlocData(i).D=D(:,deb:fin);
             BlocData(i).M=M(:,deb:fin);
             BlocData(i).Known=~isnan(BlocData(1).D);
             BlocData(i).mask=mask(deb:fin);
             BlocData(i).W1=(BlocData(i).mask*ones(1,dlen)).* BlocData(1).Known';
             BlocData(i).D(~BlocData(1).Known) = 0;
             BlocData(i).WD = 2*diag(BlocData(i).mask)*BlocData(i).D';
             BlocData(i).dconst = (((BlocData(i).D).^2)*BlocData(i).mask)';
             BlocData(i).W2 = ones(munits,1)*BlocData(i).mask'; 
             BlocData(i).D2 = ((BlocData(i).D)'.^2);
             deb=fin+1;
         end
         % initialize tracking
         start = clock;
         ...
         for t = 1:trainlen,  
             if t==1,
                 ...
                 while i0+1<=dlen, 
                     ...
                     for i=1:size(DimData,2)  
                         Dist =Dist+ ((BlocData(i).M).^2)* BlocData(i).W1(:,inds) -  BlocData(i).M* BlocData(i).WD(:,inds)+ BlocData(i).W2*(BlocData(i).D2(:,inds));
                     end 
                     [ddists(inds), bmus(inds)] = min(Dist);
                 end  
                %% Complexité de l'algorithme
                ...
                for i=1:size(DimData,2)
                    fin=sum(DimData(1:i));                    
                        switch UpdateRule,
                            case 'numeric', [ BlocData(i).M ] = UpdateNumericOM( H,P,BlocData(i).D,BlocData(i).Known);
                            case 'mixed1',
                                BlocData(i).Dquant=BlocData(i).D(:,1:vect1(1));
                                [ BlocData(i).Mquant ] = UpdateNumericOM( H,P,BlocData(i).Dquant,BlocData(i).Known(:,1:vect1(1))); 
                                BlocData(i).Dqual=BlocData(i).D(:,(vect1(1)+1):end);
                                Squal = H*(P*BlocData(i).Dqual); 
                                Aqual = H*(P*BlocData(i).Known(:,vect1(1)+1:end));
                                nonzero = find(Aqual > 0); 
                                F(nonzero) = Squal(nonzero)./ Aqual(nonzero); %% les fréquences de chaque modalité
                                F=reshape(F,size(H,1),size(BlocData(i).Dqual,2));
                                [BlocData(i).Mqual] = MqualitativeRTOM( F, H, vect2 );
                                BlocData(i).Mquant=reshape(BlocData(i).Mquant,size(H,1),size(BlocData(i).Dquant,2));
                                BlocData(i).Mqual=reshape(BlocData(i).Mqual,size(H,1),size(BlocData(i).Dqual,2));
                                BlocData(i).M=[BlocData(i).Mquant BlocData(i).Mqual];
%                             case 'mixed2',
%                                 Dquant=D(:,1:vect1(1));
%                                 [ Mquant ] = UpdateNumericOM( H,P,Dquant,Known(:,1:vect1(1))); % OK
%                                 Dqual=D(:,(vect1(1)+1):vect1(1)+sum(vect2));
