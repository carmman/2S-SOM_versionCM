function [sMap, bmus, Alpha_ck, Beta, sTrain, qe] = SS_SOM_Fast(sMap, D, varargin)
%% SS_SOM 
%[sMap bmus Alpha Beta]=SS_SOM(sMap1, D,'TypeAlgo','2SSOM','DimData',DimData,'DimBloc',DimBloc,'lambda',1,'eta',1,'trainlen',50);
% En entrée
%sMap1      :    est une carte initiale apprise avec la avec l'algorithme SOM
%           classique pour initialiser les paramètres de SOM
% D         :    la matrice des données à traiter
%'TypeAlgo' :    '2SSOM' ou SOM classique
%'DimData'  :    Contient la dimension de chaqque bloc de variables 
% DimBloc   :    Contient une structure de données spécifiant le nombre de variables par bloc   
%'lambda',et 'eta' : sont des paramètre fixé par l'utilisateur
%% Sortie 
% sMap et bmus designent respectivement la carte et bmus
% Alpha_ck et Beta les poids attribués par 2SSOM  aux blocs et aux variables
	 ...
	 % pré traitement : creation des différetentes blocs de variables  
         ...
         for t = 1:trainlen,
             ...
             if t==1,
                ...
                for i=1:size(DimData,2)
                    fin=sum(DimData(1:i));
                    [ BlocData(i).M ] = UpdateNumericOM( H,P,BlocData(i).D,BlocData(i).Known, BlocData(i).M);
...
%                  Dist=(Dist'*H)'; % prise en compte de la relation de voisinage dans le calcul des poids.  
                   [ddists(inds), bmus(inds)] = min(Dist);
                   %clear Dist;
                 end  
                %% Complexité de l'algorithme
                ...
                for i=1:size(DimData,2)
                    fin=sum(DimData(1:i));
                      [ BlocData(i).M ] = UpdateNumericOM( H,P,BlocData(i).D,BlocData(i).Known,BlocData(i).M); %ok
                    M(:,deb:fin)=BlocData(i).M;
                    deb=fin+1;
                %end
                %% Poids Alpha et Beta
                ...
                %for i=1:size(DimData,2) 
                     ...
                     %=Dist+((((PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha)).*(BlocData(i).SousBlocData(k).M).^2)* BlocData(i).SousBlocData(k).W1(:,inds) -  (PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha).*BlocData(i).SousBlocData(k).M* BlocData(i).SousBlocData(k).WD(:,inds)+ (PoidsBeta(i).SB(k).beta.*PoidsAlpha(i).SB(k).alpha).*BlocData(i).SousBlocData(k).W2*(BlocData(i).SousBlocData(k).D2(:,inds)))/DimBloc(i).Dim(k);
                     Psi_ck(:,i)=sum((DistAlpha.*H(:,bmus)),2);
                     DistAlpha=zeros(blen,dlen);
                     % les beta
                     for c=1:blen,
                               %Phi=repmat(PoidsAlpha(i).SB(k).alpha(c,1),dlen,DimBloc(i).Dim).*((repmat(H(c,bmus),DimData(i),1))').*([BlocData(i).SousBlocData(1).D]-repmat(BlocData(i).SousBlocData(1).M(c,:),dlen,1)).^2; 
                               Phi=(repmat(PoidsAlpha(i).SB(k).alpha(c,1).*H(c,bmus),DimData(i),1))' .*([BlocData(i).SousBlocData(1).D]-repmat(BlocData(i).SousBlocData(1).M(c,:),dlen,1)).^2; 
                ...
                Alpha_ck=exp(-Psi_ck/lambda)./repmat(sum(exp(-Psi_ck/lambda),2)',size(DimData,2),1)';
                     %deb=1; 
                     for i=1:size(DimData,2)
                         fin=sum(DimData(1:i));
                         debBloc=1;
